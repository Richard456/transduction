...
log_dir = 'runs/mnist_advmnist_test'
...
def data_load(eps):
    ...
    return test_dataloader_source, test_dataloader_target



def train_one_epoch(model, optimizer, dataloader_source, dataloader_target, epoch):
    model.train()
    ...
    while i < len_dataloader:
        ...
        logger.add_scalar('cls_loss', err.item(), global_step)

        i += 1

        if i % 10 == 0:
            print('epoch: %d, [iter: %d / all %d], cls_loss: %f' \
                  % (epoch, i, len_dataloader, err.cpu().data.numpy()))
...
if __name__ == "__main__":
    ...
    model = torch.load(os.path.join(model_root, "mnist_advmnist.pth"))
    model = model.cuda()

    # freeze model but predictor
    for param in model.feature:
        param.requires_grad = False
    for param in model.discriminator:
        param.requires_grad = False

    # training on test dataset
    optimizer = optim.Adam(model.parameters(), lr=lr)
    for epoch in range(n_epoch):
        train_one_epoch(model, optimizer, test_dataloader_source, test_dataloader_target, epoch)
        # test(model, test_dataloader_source, source_dataset_name, epoch)
        # test(model, test_dataloader_target, target_dataset_name, epoch)

    torch.save(model, '{0}/mnist_advmnist_test.pth'.format(model_root))
